...
def process_event(event, endpoint):
    ...
    # and store it in a variable named event_dict
    event_dict = {
        'type': endpoint,
    ...
    producer.produce(json.dumps(event_dict).encode('utf-8'))
    
    # TODO: log "PRODUCER::producing x event" where x is the actual event type
    logger.info(f'PRODUCER::producing {event_dict["payload"]} event')
